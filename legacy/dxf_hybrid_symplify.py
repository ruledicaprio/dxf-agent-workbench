...
def simplify_mesh(mesh: trimesh.Trimesh, target_faces: int) -> trimesh.Trimesh:
    ...
    # Stage 1 – cheap reduction
    if len(mesh.faces) > 350_000:
        print("[*] Stage 1: First reduction (low RAM)...")
    ...
    # Stage 2 – final reduction with better feedback
    print(f"[*] Stage 2: Final quadric → target {target_faces:,} faces...")
    print("    (this is the slow part – usually 3–12 minutes on complex genset meshes)")
    print("    Working... please wait")

    # Force trimesh path (more predictable memory + usually faster than Open3D on 16 GB)
