...
def simplify_mesh(mesh: trimesh.Trimesh, target_faces: int) -> trimesh.Trimesh:
    ...
    # Light cleaning only (important for Meshy meshes)
    mesh.merge_vertices()
    ...
    # ---------- Stage 1: Fast aggressive reduction ----------
    # This gives early feedback and dramatically reduces memory pressure
    stage1_target = min(180_000, max(target_faces * 6, 80_000))

    print(f"[*] Stage 1: Fast reduction → ~{stage1_target:,} faces (aggression=9)")
    t0 = time.time()
    ...
    print(f"[*] Stage 1 done in {time.time()-t0:.1f}s → {len(mesh.faces):,} faces")

    if len(mesh.faces) <= target_faces:
        return mesh

    # ---------- Stage 2: Quality pass with pyfqmr ----------
    print(f"[*] Stage 2: pyfqmr quality pass → {target_faces:,} faces (aggressiveness=6)")
